...
var serverFunc = function (req, res) {
	...
	/* Incoming http options */
		...
		res.setHeader('Access-Control-Max-Age', 600); // il client può inviare al server altre richieste POST per 5 minuti prima di dovergli mandare nuovamente una richiesta OPTIONS
		res.writeHead(200);
		res.end();
	}
	/* End http options */
	/* Incoming not expected messagge */
	else {
		console.log('Metodo ' + req.method + ' non previsto');
		res.writeHead(404);
		res.end();
	}
	/* End not expected messagge */
}
